...
(function () {
  const EN = {
    ...
    "AI 生成提交信息（基于暂存/未提交 diff）": "AI-generate commit message (from staged/uncommitted diff)",
    "生成提交信息失败：": "Failed to generate commit message: ",
    "没有可生成提交信息的改动": "No changes to generate a commit message from",
    "生成结果为空": "Generation returned nothing",
    ...
    // —— 底部状态栏 ——
    "已同步": "Synced",
    "当前分支 · 点击查看源代码管理": "Current branch · click to open Source Control",
    "同步：先拉取再推送（↓落后 ↑领先）": "Sync: pull then push (↓ behind ↑ ahead)",
    "工作区改动 · 点击查看": "Working tree changes · click to view",
    "命令面板（⌘/Ctrl+K · 文件用 ⌘/Ctrl+P）": "Command palette (⌘/Ctrl+K · files via ⌘/Ctrl+P)",
    "快速打开文件（命令面板）": "Quick open file (command palette)",
    "全文搜索文件内容": "Search across file contents",
    ...
    "生成代码地图": "Generate Codemap",
    "代码地图": "Codemap",
    "代码地图生成中…": "Codemap is being generated…",
    "正在分析代码结构，生成代码地图…（约 1-3 分钟）": "Analyzing code structure to build the codemap… (about 1-3 min)",
