...
**并发模型（合并写 + 文件监听双向同步）**：
- 桌面端保存时先读文件做**合并**（文件里有、本端没有且未被本端删除的对话/归档保留），不再整份盲覆盖；本端删除过的 id 记在渲染层会话内存（`removed`，不落盘），防止被合并"复活"。
- 主进程 `fs.watch` 监听 data 目录中该文件，手机端写入后通知渲染层 `convs:changed`，桌面端把新对话/新归档合并进界面；已打开且空闲的对话若内容有变（手机续聊）会刷新，**回复中的对话不动**。
- 已知边界：删除不跨端传播（一端删了另一端还在，会被同步回来）；两端同时改"同一个"对话仍是后写为准。
